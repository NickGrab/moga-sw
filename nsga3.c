...
int N = 20; // size of  population
int nvar=5; // number of variables to be manipulated
int nobj = 3; // number of objectives 
double crossover_P=0.9; // crossover percentage of population
double mutation_P=0.1; // mutation percentage of population
double mutation_rate = 0.02; // rate of mutation
...
int main(int argc, char *argv[]) {
    ...
    if(access(old_input, F_OK) != -1) { // returns TRUE is file exists
...
int normalize(double obj[nobj][2*N],double norm_obj[nobj][2*N]) {
    ...
    for(i=0;i<nobj;i++) { // loop through objective axes
        ...
        for(j=0;j<2*N;j++) { // finds index of the minimum of the set of maximums
            if(j==0) {
                for(k=0;k<i;k++) {
                    if(j==index[k]) {
                        repeat = 1;
...
int niching(int pop_filled,int front[2*N],int nearest_pointer[2*N], int new_pop_point[N], double nearest_distance[2*N]) {
    ...
    while(pop_filled<N) { // loops until parent population is... populated
        ...
        for(j=1;j<size_hyperplane;j++) {
            if(rho[j]==niche_min) {
                num_niche_min += 1; // increments for multiple niches of smallest size
            }
            if(rho[j]<niche_min) {
            ...
            for(j=0;j<2*N;j++) { // loop over population
                if(front[j]==1){ // if point is member of the front
                    ...
                    if(nearest_pointer[j]==niche_point) { 
                        if(initialized == 0) { // if this is first member of niche
